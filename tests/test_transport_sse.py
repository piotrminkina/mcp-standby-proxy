...
def _make_mock_sse_context(client_send, client_recv):
    """Return a mock sse_client context that yields (client_recv, client_send).
...
async def test_connect_sets_connected() -> None:
    ...
    with patch(
        "mcp_standby_proxy.transport.sse.sse_client",
        _make_mock_sse_context(client_recv, client_send),
    ):
...
async def test_request_closed_stream_raises_transport_error() -> None:
    ...
    @asynccontextmanager
    async def _ctx(url, **kwargs):
        yield server_recv, closed_write
...
async def test_close_sets_disconnected() -> None:
    ...
    @asynccontextmanager
    async def _ctx(url, **kwargs):
        yield client_recv, client_write
